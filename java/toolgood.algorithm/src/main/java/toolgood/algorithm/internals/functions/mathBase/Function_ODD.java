...
public final class Function_ODD extends Function_1 {
    public Function_ODD(FunctionBase[] funcs) {
        super(funcs);
        if (funcs.length != 1) {
    ...
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        if (z.compareTo(BigDecimal.ZERO) == 0) {
            return Operand.Create(BigDecimal.ONE);
        }

        if (z.compareTo(BigDecimal.ZERO) > 0) {
            if (z.remainder(BigDecimal.valueOf(2)).abs().compareTo(BigDecimal.ONE) == 0) {
                return args1;
            }
