...
final class Function_SUBSTITUTE extends Function_4 {
    public Function_SUBSTITUTE(FunctionBase[] funcs) {
        super(funcs);
        if (funcs.length < 3 || funcs.length > 4) {
            throw new IllegalArgumentException("Function '" + Name() + "' requires 3 to 4 parameters.");
        }
    }
    ...
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        while (searchPos <= text.length() - oldtext.length()) {
            ...
            if (foundCount == replaceIndex) {
                sb.append(text, currentIndex, foundPos);
                sb.append(newtext);
