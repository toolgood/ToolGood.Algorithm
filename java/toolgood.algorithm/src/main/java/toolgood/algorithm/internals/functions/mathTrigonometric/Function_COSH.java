...
public final class Function_COSH extends Function_1 {
    public Function_COSH(FunctionBase[] funcs) {
        super(funcs);
        if (funcs.length != 1) {
    ...
    @Override
    public String Name() { return "Cosh"; }
    ...
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        // double 上限约 1.8e308,|x|>=709 时 Math.cosh 返回 Infinity
        if (x >= 709 || x <= -709) { return ParameterError(1); }
        return Operand.Create(BigDecimal.valueOf(Math.cosh(x)));
    }
