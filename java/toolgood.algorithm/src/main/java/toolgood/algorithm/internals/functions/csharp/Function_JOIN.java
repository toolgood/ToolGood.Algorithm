...
public final class Function_JOIN extends Function_N {
    ...
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        if (args1.IsArray()) {
            List<String> list = new ArrayList<String>(args1.ArrayValue().size());
            boolean o = FunctionUtil.FlattenToStringList(args1, list);
            ...
            for (int i = 1; i < args.size(); i++) {
                boolean o = FunctionUtil.FlattenToStringList(args.get(i), list);
