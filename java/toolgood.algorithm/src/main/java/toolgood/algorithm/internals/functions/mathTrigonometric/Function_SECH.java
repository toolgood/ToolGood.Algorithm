...
public final class Function_SECH extends Function_1 {
    public Function_SECH(FunctionBase[] funcs) {
        super(funcs);
        if (funcs.length != 1) {
    ...
    @Override
    public String Name() { return "Sech"; }
    ...
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        // sech 在 |x| 较大时趋近 0,提前返回避免 Infinity
        if (x >= 709 || x <= -709) { return Operand.Create(BigDecimal.ZERO); }
        return Operand.Create(BigDecimal.valueOf(1.0 / Math.cosh(x)));
    }
