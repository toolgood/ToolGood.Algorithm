...
final class Function_SEC extends Function_1 {
    public Function_SEC(FunctionBase[] funcs) {
        super(funcs);
        if (funcs.length != 1) {
    ...
    @Override
    public String Name() { return "Sec"; }

    @Override
    public Operand Evaluate(AlgorithmEngine engine, BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        Operand args1 = GetNumber_1(engine, tempParameter);
        if (args1.IsErrorOrNone()) { return args1; }
        double d = Math.cos(args1.NumberValue().doubleValue());
        if (d == 0) {
            return Div0Error();
        }
