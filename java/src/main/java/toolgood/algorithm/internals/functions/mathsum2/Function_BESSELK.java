...
public final class Function_BESSELK extends Function_2 {
    ...
    private static BigDecimal BesselK0(BigDecimal x) {
        if (x.compareTo(new BigDecimal("2")) <= 0) {
            ...
            BigDecimal t1 = y1.multiply(new BigDecimal("0.000007400456812"));
            BigDecimal t2 = y1.multiply(new BigDecimal("0.000107502176243").add(t1));
            BigDecimal t3 = y1.multiply(new BigDecimal("0.002626979711643").add(t2));
            BigDecimal t4 = y1.multiply(new BigDecimal("0.034885890266341").add(t3));
            BigDecimal t5 = y1.multiply(new BigDecimal("0.230697567077446").add(t4));
            BigDecimal t6 = y1.multiply(new BigDecimal("0.42278433509846713939").add(t5));
            BigDecimal t7 = new BigDecimal("-0.57721566490153286061").add(t6);
            BigDecimal ans = MathEx.Log(x.divide(new BigDecimal("2"), java.math.MathContext.DECIMAL128)).negate().multiply(BesselI0(x)).add(t7);
            return ans;
        }
        BigDecimal y2 = new BigDecimal("2").divide(x, java.math.MathContext.DECIMAL128);

        BigDecimal a1 = y2.multiply(new BigDecimal("0.000532080305632"));
        BigDecimal a2 = y2.multiply(new BigDecimal("-0.002515401617640").add(a1));
        BigDecimal a3 = y2.multiply(new BigDecimal("0.005878072214632").add(a2));
        BigDecimal a4 = y2.multiply(new BigDecimal("-0.010624628097740").add(a3));
        BigDecimal a5 = y2.multiply(new BigDecimal("0.021895687854228").add(a4));
        BigDecimal a6 = y2.multiply(new BigDecimal("-0.078323582855262").add(a5));
        BigDecimal a7 = new BigDecimal("1.253314137315500").add(a6);

        BigDecimal ans2 = MathEx.Exp(x.negate()).divide(MathEx.Sqrt(x), java.math.MathContext.DECIMAL128).multiply(a7);
        return ans2;
    }
    ...
    private static BigDecimal BesselK1(BigDecimal x) {
        if (x.compareTo(new BigDecimal("2")) <= 0) {
            BigDecimal y1 = x.multiply(x).divide(new BigDecimal("4"), java.math.MathContext.DECIMAL128);
            BigDecimal t1 = y1.multiply(new BigDecimal("0.000046862429868"));
            BigDecimal t2 = y1.multiply(new BigDecimal("0.001104044918568").add(t1));
            BigDecimal t3 = y1.multiply(new BigDecimal("-0.019194020400716").add(t2));
            BigDecimal t4 = y1.multiply(new BigDecimal("-0.181568943578864").add(t3));
            BigDecimal t5 = y1.multiply(new BigDecimal("-0.672785797513523").add(t4));
            BigDecimal t6 = y1.multiply(new BigDecimal("0.154431442036717").add(t5));
            BigDecimal t7 = BigDecimal.ONE.add(t6);
            BigDecimal ans = MathEx.Log(x.divide(new BigDecimal("2"), java.math.MathContext.DECIMAL128)).multiply(BesselI1(x)).add(BigDecimal.ONE.divide(x, java.math.MathContext.DECIMAL128).multiply(t7));
            return ans;
        }
        BigDecimal y2 = new BigDecimal("2").divide(x, java.math.MathContext.DECIMAL128);

        BigDecimal a1 = y2.multiply(new BigDecimal("-0.000682450383692"));
        BigDecimal a2 = y2.multiply(new BigDecimal("0.003256142832609").add(a1));
        BigDecimal a3 = y2.multiply(new BigDecimal("-0.007803534366237").add(a2));
        BigDecimal a4 = y2.multiply(new BigDecimal("0.015042680553908").add(a3));
        BigDecimal a5 = y2.multiply(new BigDecimal("-0.036556202034020").add(a4));
        BigDecimal a6 = y2.multiply(new BigDecimal("0.234986192707248").add(a5));
        BigDecimal a7 = new BigDecimal("1.253314137315500").add(a6);
