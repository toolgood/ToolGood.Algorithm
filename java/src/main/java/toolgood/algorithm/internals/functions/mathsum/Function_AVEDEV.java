...
public final class Function_AVEDEV extends Function_N {
    ...
    public Operand Evaluate(AlgorithmEngine engine, java.util.function.BiFunction<AlgorithmEngine, String, Operand> tempParameter) {
        ...
        BigDecimal avg = sum.divide(new BigDecimal(list.size()), java.math.MathContext.DECIMAL128);
        BigDecimal sum2 = BigDecimal.ZERO;
        for (int i = 0; i < list.size(); i++) {
            sum2 = sum2.add(list.get(i).subtract(avg).abs());
        }
        return Operand.Create(sum2.divide(new BigDecimal(list.size()), java.math.MathContext.DECIMAL128));
    }
