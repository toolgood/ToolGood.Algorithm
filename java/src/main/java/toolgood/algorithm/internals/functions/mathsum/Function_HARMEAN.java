...
public final class Function_HARMEAN extends Function_N {
    ...
    public Operand Evaluate(AlgorithmEngine engine, java.util.function.BiFunction<AlgorithmEngine, String, Operand> tempParameter) throws Exception {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            sum = sum.add(BigDecimal.ONE.divide(list.get(i), java.math.MathContext.DECIMAL128));
        }
        if (sum.compareTo(BigDecimal.ZERO) == 0) {
            return FunctionError();
        }
        return Operand.Create(new BigDecimal(list.size()).divide(sum, java.math.MathContext.DECIMAL128));
    }
