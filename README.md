...
注：ToolGood.Algorithm主要以`Decimal`为基础，`Decimal`精度高，不代表没有误差，速度较`double`慢。
...
2）项目维护时，经常改动的算法，不想硬编码的代码；

3）财务数据、统计数据之中的算法，(注:`Decimal`精度高，也有误差，建议使用`分`为单位)；
...
注4：`json数组索引`也受`Excel索引`影响，如`[1,2,3][1]`在Excel索引模式下返回第一个元素`1`，在C#索引模式下返回第二个元素`2`。
...
<table>
    ...
    <tr>
        <td>IFS</td><td>ifs(条件1, 值1, 条件2, 值2, ...)<br>检查是否满足一个或多个条件, 且返回第一个TRUE条件对应的值。</td>
        <td>IFS(1=1, 'a', 1=2, 'b') <br>>>a</td>
    </tr>
    <tr>
        <td>SWITCH</td><td>switch(表达式, 值1, 结果1, 值2, 结果2, ...)<br>根据值列表计算表达式的值, 并返回对应结果。</td>
    ...
    <tr>
        <td>XOR</td><td>xor(逻辑值1, ...)<br>返回所有参数的异或值, 奇数个TRUE返回TRUE</td>
...
<table>
   ...
   <tr>
    ...
    <tr>
        <td>combin</td><td>combin(总数, 选取数)<br>计算从给定数目的对象集合中提取若干对象的组合数</td>
        <td>combin(10, 2) <br>>>45</td>
    </tr>
    <tr>
        <td>PERMUT</td><td>permut(总数, 选取数)<br>返回从给定数目的对象集合中选取的若干对象的排列数</td>
   ...
   <tr>
    ...
    <tr>
        <td>COT</td><td>cot(弧度)<br>返回数值的余切值</td>
        <td>cot(1) <br>>>0.64209261593</td>
    </tr>
    <tr>
        <td>COTH</td><td>coth(弧度)<br>返回数值的双曲余切值</td>
        <td>coth(1) <br>>>1.31303528549</td>
    </tr>
    <tr>
        <td>CSC</td><td>csc(弧度)<br>返回数值的余割值</td>
        <td>csc(1) <br>>>1.18839510578</td>
    </tr>
    <tr>
        <td>CSCH</td><td>csch(弧度)<br>返回数值的双曲余割值</td>
        <td>csch(1) <br>>>0.85091812823</td>
    </tr>
    <tr>
        <td>SEC</td><td>sec(弧度)<br>返回数值的正割值</td>
        <td>sec(1) <br>>>1.85081571768</td>
    </tr>
    <tr>
        <td>SECH</td><td>sech(弧度)<br>返回数值的双曲正割值</td>
        <td>sech(1) <br>>>0.64805427366</td>
    </tr>
    <tr>
        <td>ACOT</td><td>acot(数值)<br>返回数值的反余切值</td>
        <td>acot(1) <br>>>0.78539816339</td>
    </tr>
    <tr>
        <td>ACOTH</td><td>acoth(数值)<br>返回数值的反双曲余切值</td>
    ...
    <tr>
        <td>ERF</td><td>erf(数值)<br>返回误差函数值</td>
        <td>ERF(1) <br>>>0.842701</td>
    </tr>
    <tr>
        <td>ERFC</td><td>erfc(数值)<br>返回补余误差函数值</td>
        <td>ERFC(1) <br>>>0.157299</td>
    </tr>
    <tr>
        <td>BESSELI</td><td>besselI(x, n)<br>返回修正贝塞尔函数 In(x)</td>
        <td>BESSELI(1.5, 1) <br>>>0.981666</td>
    </tr>
    <tr>
        <td>BESSELJ</td><td>besselJ(x, n)<br>返回贝塞尔函数 Jn(x)</td>
        <td>BESSELJ(1.5, 1) <br>>>0.557937</td>
    </tr>
    <tr>
        <td>BESSELK</td><td>besselK(x, n)<br>返回修正贝塞尔函数 Kn(x)</td>
        <td>BESSELK(1.5, 1) <br>>>0.277388</td>
    </tr>
    <tr>
        <td>BESSELY</td><td>besselY(x, n)<br>返回贝塞尔函数 Yn(x)</td>
        <td>BESSELY(1.5, 1) <br>>>-0.412309</td>
    </tr>
    <tr>
        <td>DELTA</td><td>delta(数值1[, 数值2])<br>检验两个数值是否相等, 相等返回1, 否则返回0</td>
        <td>DELTA(5, 5) <br>>>1</td>
    </tr>
    <tr>
        <td>GESTEP</td><td>gestep(数值[, 阈值])<br>检验数值是否大于等于阈值, 大于等于返回1, 否则返回0</td>
    ...
    <tr>
        <td>SUMPRODUCT</td><td>sumproduct(数组1, 数组2, ...)<br>返回数组对应元素乘积之和</td>
        <td>SUMPRODUCT(array(1, 2, 3), array(4, 5, 6)) <br>>>32</td>
    </tr>
    <tr>
        <td>SUMX2MY2</td><td>sumx2my2(数组X, 数组Y)<br>返回两数组中对应数值的平方差之和</td>
        <td>SUMX2MY2(array(1, 2, 3), array(4, 5, 6)) <br>>>-63</td>
    </tr>
    <tr>
        <td>SUMX2PY2</td><td>sumx2py2(数组X, 数组Y)<br>返回两数组中对应数值的平方和之和</td>
        <td>SUMX2PY2(array(1, 2, 3), array(4, 5, 6)) <br>>>91</td>
    </tr>
    <tr>
        <td>SUMXMY2</td><td>sumxmy2(数组X, 数组Y)<br>返回两数组中对应数值之差的平方和</td>
        <td>SUMXMY2(array(1, 2, 3), array(4, 5, 6)) <br>>>27</td>
    </tr>
    <tr>
        <td>SERIESSUM</td><td>seriessum(X, N, M, 系数数组)<br>返回幂级数之和</td>
        <td>SERIESSUM(2, 0, 1, array(1, 1, 1, 1)) <br>>>15</td>
    </tr>
    <tr>
        <td>ARABIC ★</td><td>arabic(罗马数字文本)<br>将罗马数字转换为阿拉伯数字</td>
        <td>ARABIC('MMXXIII') <br>>>2023</td>
    </tr>
    <tr>
        <td>ROMAN ★</td><td>roman(数值[, 类型])<br>将阿拉伯数字转换为罗马数字</td>
...
<table>
    ...
    <tr>
        <td>UNICHAR ★</td><td>unichar(数值)<br>返回由Unicode数值指定的字符</td>
        <td>unichar(65) <br>>>A</td>
    </tr>
    <tr>
        <td>UNICODE ★</td><td>unicode(字符串)<br>返回文本字符串中第一个字符的Unicode数值代码</td>
...
<table>
    ...
    <tr>
        <td>DAYS</td><td>days(结束日期, 开始日期)<br>返回两个日期之间的天数</td>
        <td>DAYS("2017-1-7", "2017-1-1") <br>>>6</td>
    </tr>
    <tr>
        <td>YEARFRAC</td><td>yearfrac(开始日期, 结束日期[, 基准类型：0-4])<br>返回两个日期之间的天数占全年天数的比例</td>
...
<table>
    <tr><td>函数名</td><td>说明</td><td>示例</td></tr>
    <tr>
        <td>PMT</td><td>pmt(利率, 期数, 现值[, 终值[, 类型]])<br>计算在定期支付、固定利率的情况下贷款的每期付款额</td>
        <td>PMT(0.08/12, 10, 10000) <br>>>-1037.03</td>
    </tr>
    <tr>
        <td>PPMT</td><td>ppmt(利率, 期数, 总期数, 现值[, 终值[, 类型]])<br>计算在定期支付、固定利率的情况下贷款的每期本金付款额</td>
        <td>PPMT(0.08/12, 1, 10, 10000) <br>>>-970.37</td>
    </tr>
    <tr>
        <td>IPMT</td><td>ipmt(利率, 期数, 总期数, 现值[, 终值[, 类型]])<br>计算在定期支付、固定利率的情况下贷款的每期利息付款额</td>
        <td>IPMT(0.08/12, 1, 10, 10000) <br>>>-66.67</td>
    </tr>
    <tr>
        <td>PV</td><td>pv(利率, 期数, 每期付款[, 终值[, 类型]])<br>计算投资的现值</td>
        <td>PV(0.08/12, 10, -1000) <br>>>9637.09</td>
    </tr>
    <tr>
        <td>FV</td><td>fv(利率, 期数, 每期付款[, 现值[, 类型]])<br>计算投资的终值</td>
        <td>FV(0.08/12, 10, -1000) <br>>>10413.84</td>
    </tr>
    <tr>
        <td>NPER</td><td>nper(利率, 每期付款, 现值[, 终值[, 类型]])<br>计算投资的期数</td>
        <td>NPER(0.08/12, -1000, 10000) <br>>>10.7</td>
    </tr>
    <tr>
        <td>RATE</td><td>rate(期数, 每期付款, 现值[, 终值[, 类型[, 猜测值]]])<br>计算投资的每期利率</td>
        <td>RATE(4*12, -200, 8000) <br>>>0.0077</td>
    </tr>
    <tr>
        <td>NPV</td><td>npv(利率, 数值1, ...)<br>计算一系列定期现金流的净现值</td>
        <td>NPV(0.1, -10000, 3000, 4200, 6800) <br>>>1188.44</td>
    </tr>
    <tr>
        <td>XNPV</td><td>xnpv(利率, 现金流数组, 日期数组)<br>计算一系列非定期现金流的净现值</td>
        <td>XNPV(0.09, array(-10000, 2750, 4250, 3250, 2750), array('2008-1-1', '2008-3-1', '2008-10-30', '2009-2-15', '2009-4-1')) <br>>>2086.65</td>
    </tr>
    <tr>
        <td>IRR</td><td>irr(现金流数组[, 猜测值])<br>计算一系列现金流的内部收益率</td>
        <td>IRR(array(-70000, 12000, 15000, 18000, 21000, 26000)) <br>>>0.0866</td>
    </tr>
    <tr>
        <td>MIRR</td><td>mirr(现金流数组, 融资利率, 再投资利率)<br>计算一系列现金流的修正内部收益率</td>
        <td>MIRR(array(-70000, 12000, 15000, 18000, 21000, 26000), 0.1, 0.12) <br>>>0.0797</td>
    </tr>
    <tr>
        <td>XIRR</td><td>xirr(现金流数组, 日期数组[, 猜测值])<br>计算一系列非定期现金流的内部收益率</td>
        <td>XIRR(array(-10000, 2750, 4250, 3250, 2750), array('2008-1-1', '2008-3-1', '2008-10-30', '2009-2-15', '2009-4-1')) <br>>>0.3734</td>
    </tr>
    <tr>
        <td>SLN</td><td>sln(原值, 残值, 使用年限)<br>计算资产的直线折旧</td>
        <td>SLN(30000, 7500, 10) <br>>>2250</td>
    </tr>
    <tr>
        <td>DB</td><td>db(原值, 残值, 使用年限, 期数[, 月数])<br>计算资产的余额递减折旧</td>
        <td>DB(1000000, 100000, 6, 1) <br>>>319000</td>
    </tr>
    <tr>
        <td>DDB</td><td>ddb(原值, 残值, 使用年限, 期数[, 余额递减率])<br>计算资产的双倍余额递减折旧</td>
        <td>DDB(2400, 300, 10, 2) <br>>>384</td>
    </tr>
    <tr>
        <td>SYD</td><td>syd(原值, 残值, 使用年限, 期数)<br>计算资产的年数总和折旧</td>
...
<table>
    ...
    <tr>
        <td>LookCeiling</td><td>lookCeiling(数值, 数组)<br>在数组中查找大于等于指定值的最小值</td>
        <td>LookCeiling(2.5, [1, 2, 3, 4, 5]) <br>>>3</td>
    </tr>
    <tr>
        <td>LookFloor</td><td>lookFloor(数值, 数组)<br>在数组中查找小于等于指定值的最大值</td>
        <td>LookFloor(2.5, [1, 2, 3, 4, 5]) <br>>>2</td>
    </tr>
    <tr>
        <td>ARRAY</td><td>array(值1, 值2, ...)<br>创建数组</td>
    ...
    <tr>
        <td>RANK</td><td>rank(数值, 数组[, 排序方式：0/1])<br>返回数值在数组中的排名</td>
        <td>RANK(3, array(1, 2, 3, 4, 5)) <br>>>3</td>
    </tr>
    <tr>
        <td>FORECAST</td><td>forecast(X, Y数组, X数组)<br>根据线性趋势返回预测值</td>
        <td>FORECAST(30, array(6, 7, 9, 15, 21), array(20, 28, 31, 38, 40)) <br>>>10.61</td>
    </tr>
    <tr>
        <td>INTERCEPT</td><td>intercept(Y数组, X数组)<br>返回线性回归线的截距</td>
        <td>INTERCEPT(array(2, 3, 9, 1, 8), array(6, 5, 11, 7, 5)) <br>>>0.05</td>
    </tr>
    <tr>
        <td>SLOPE</td><td>slope(Y数组, X数组)<br>返回线性回归线的斜率</td>
        <td>SLOPE(array(2, 3, 9, 1, 8), array(6, 5, 11, 7, 5)) <br>>>0.67</td>
    </tr>
    <tr>
        <td>CORREL</td><td>correl(数组1, 数组2)<br>返回两个数组之间的相关系数</td>
        <td>CORREL(array(3, 2, 4, 5, 6), array(9, 7, 12, 15, 17)) <br>>>0.997</td>
    </tr>
    <tr>
        <td>PEARSON</td><td>pearson(数组1, 数组2)<br>返回Pearson乘积矩相关系数</td>
...
<table>
    <tr><td>函数名</td><td>说明</td><td>示例</td></tr>
    <tr>
        <td>LookFloor</td><td>LookFloor(值, 数组)<br>查找下限值。数组会被排序，高于最大值，取最大值，低于最小值，取最小值</td> <td></td>
    </tr>
    <tr>
        <td>LookCeiling</td><td>LookCeiling(值, 数组)<br>查找上限值。数组会被排序，高于最大值，取最大值，低于最小值，取最小值</td> <td></td>
    </tr>
</table>
