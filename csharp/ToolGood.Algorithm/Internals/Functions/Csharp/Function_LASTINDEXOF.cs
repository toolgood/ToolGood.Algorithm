...
namespace ToolGood.Algorithm.Internals.Functions.Csharp
{
	internal sealed class Function_LASTINDEXOF : Function_4
	{
		...
		public override Operand Evaluate(AlgorithmEngine engine, Func<AlgorithmEngine, string, Operand> tempParameter)
		{
			...
			if(func4 == null) {
				return Operand.Create(text.LastIndexOf(args2.TextValue, startIndex, StringComparison.Ordinal) + engine.ExcelIndex);
			}
			...
			return Operand.Create(text.LastIndexOf(args2.TextValue, startIndex, count, StringComparison.Ordinal) + engine.ExcelIndex);
		}
