...
namespace ToolGood.Algorithm.Internals.Functions
{
	...
	internal class Function_AND_N : Function_N
	{
		...
		public override Operand Evaluate(AlgorithmEngine work, Func<string, Operand> tempParameter)
		{
			...
			foreach (var item in funcs) {
				var a = item.Evaluate(work, tempParameter).ToBoolean("Function '{0}' parameter {1} is error!", "AND", index++);
	...
	internal class Function_OR_N : Function_N
	{
		...
		public override Operand Evaluate(AlgorithmEngine work, Func<string, Operand> tempParameter)
		{
			...
			bool b = false;
			foreach (var item in funcs) {
				var a = item.Evaluate(work, tempParameter).ToBoolean("Function '{0}' parameter {1} is error!", "OR", index++);
				if (a.IsError) { return a; }
