...
{
    public class mathLexer2 : ITokenSource
    {
        public const int
        ...
        public IToken NextToken()
        {
            while (true)
            {
                ...
                {
                    var eofToken = TokenFactory.Create(Tuple.Create((ITokenSource)this, (ICharStream)_input), TokenConstants.EOF, "", TokenConstants.DefaultChannel, _input.Index, _input.Index - 1, _line, _column);
                    return eofToken;
                }
                ...
                if (IsDigit(c) || (c == '.' && IsDigit(_input.LA(2))))
                {
                    return ReadNumber();
                }
        ...
        private IToken CreateToken(int type)
        {
            ...
            return TokenFactory.Create(Tuple.Create((ITokenSource)this, (ICharStream)_input), type, text, TokenConstants.DefaultChannel, _startCharIndex, stopIndex, _startLine, _startColumn);
        }
        ...
        {
            return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_' || IsFullWidthLetter(c);
        }
        ...
        private bool IsFullWidthLetter(int c)
        {
            return (c >= '\u00c0' && c <= '\u00d6') ||
                   (c >= '\u00d8' && c <= '\u00f6') ||
                   (c >= '\u00f8' && c <= '\u00ff') ||
        ...
        private void ConsumeLineComment()
        {
            ...
            {
                int c = _input.LA(1);
                if (c == IntStreamConstants.EOF || c == '\r' || c == '\n') break;
                _input.Consume();
            }
        }
