...
class Function_COMBIN extends Function_2 {
    ...
    Evaluate(engine, tempParameter) {
        ...
        if (args1.IsNotNumber) {
            args1 = args1.ToNumber(StringCache.Function_parameter_error2, "Combin", 1);
            if (args1.IsError) { return args1; }
        }
        let args2 = this.func2.Evaluate(engine, tempParameter);
        if (args2.IsNotNumber) {
            args2 = args2.ToNumber(StringCache.Function_parameter_error2, "Combin", 2);
        ...
        if (total < 0 || count < 0 || total < count) {
            return Operand.Error(StringCache.Function_parameter_error2, "Combin");
        }
        let sum = 1;
        let sum2 = 1;
