...
class Function_WEIBULL extends Function_4 {
    ...
    Evaluate(engine, tempParameter) {
        ...
        if (args1.IsNotNumber) {
            let converted1 = args1.ToNumber("Function {0} parameter {1} is error!", "Weibull", 1);
            if (converted1.IsError) return converted1;
            args1 = converted1;
        }
        let args2 = this.func2.Evaluate(engine, tempParameter);
        if (args2.IsNotNumber) {
            let converted2 = args2.ToNumber("Function {0} parameter {1} is error!", "Weibull", 2);
            if (converted2.IsError) return converted2;
            args2 = converted2;
        }
        let args3 = this.func3.Evaluate(engine, tempParameter);
        if (args3.IsNotNumber) {
            let converted3 = args3.ToNumber("Function {0} parameter {1} is error!", "Weibull", 3);
            if (converted3.IsError) return converted3;
            args3 = converted3;
        }
        let args4 = this.func4.Evaluate(engine, tempParameter);
        if (args4.IsNotBoolean) {
            let converted4 = args4.ToBoolean("Function {0} parameter {1} is error!", "Weibull", 4);
